...
File_Dat5="./Input/Weight_distribution_%s_%s.dat" %(Which_network,initial_nodes)
...
if(Which_network=='BARABASI'):
	...
	for u,v,d in sorted(H.edges(data=True)): #u,v -> pairs, d=weight
		...
		d['weight']=1./np.sqrt(H.out_degree(u)*H.in_degree(v))
		#d['weight']=1./np.sqrt(H.degree(u)*H.degree(v))
		d_hist.append(d['weight'])
		#print H.in_degree(u)
		#print >> fout3,(H.in_degree(v))
	np.savetxt(fout1,nx.to_numpy_matrix(H),'%5.3f')
	np.savetxt(fout2,nx.to_numpy_matrix(H),'%5.3f')
	
	#np.savetxt(fout1,nx.adjacency_matrix(H),'%5.3f')
	#np.savetxt(fout2,nx.adjacency_matrix(H),'%5.3f')
	weights = np.ones_like(d_hist)/len(d_hist)
	n, bins,patches=pylab.hist(d_hist,weights=weights)
	pylab.xlim([0,1.0])
	pylab.ylim([0,1.0])
	pylab.xlabel('Coupling strengths',fontsize=20)
	pylab.ylabel('Probability',fontsize=20,labelpad=9)
	pylab.grid(True)
	pylab.tick_params(axis='both', which='major', labelsize=25,labelbottom=5)
	pylab.savefig('./Input/Histogram_%s.eps' %initial_nodes)
	pylab.close()
	#nx.draw(H)
