...
if Attractor == 'Chen':
	...
	Thresh = 0.0#2.75
elif Attractor == 'Noise':
	Thresh = 0.0

fout=open('%s/Poincare_Section_Maxima_%s_%03i.dat' %(path,Attractor,Seed),'w')
fout2=open('%s/Position_Peaks_Poincare_Section_Maxima_SEED_%s_Thresh_%s.dat' %(path,Seed,Thresh),'w')
...
for infile in infiles:
	...
	#print Thresh*std(Val_Max)
	for k in range(0,len(Pos_Max)):
		if(Val_Max[k]>Thresh*std(Val_Max)): 
	...
	for l in range(len(Real_Spikes1)):
		if(Real_Spikes1[l]-Real_Spikes1[l-1]>0.0):
			...
			print >> fout,(Real_Spikes1[l-1]-Real_Spikes1[l-2])*1,(Real_Spikes1[l]-Real_Spikes1[l-1])*1
